defmodule Guerilla.Gateway.Client.Allocator do
  @moduledoc """
  ...
  def handle_call({:allocate, name, host, address_family}, _from, state(free_ports: []) = state) do
    Logger.warn("Client allocate: No free port available, unable to allocate for #{name}@#{host} (#{address_family})")
  ...
  def handle_call({:allocate, name, host, address_family}, _from, state(free_ports: [next_free_port|remaining_free_ports]) = state) do
    Logger.info("Client allocate: Allocating #{next_free_port} for #{name}@#{host} (#{address_family})")
